...
# Custom CSS สำหรับธีม ขาว-แดง-ดำ
st.markdown("""
    <style>
    .main { background-color: #ffffff; }
    .stMetric { background-color: #f8f9fa; border-radius: 10px; padding: 15px; border-left: 5px solid #dc3545; }
    div.stButton > button:first-child { background-color: #dc3545; color: white; border-radius: 5px; }
...
with st.sidebar:
    ...
    st.divider()
    uploaded_file = st.file_uploader("📷 Upload Sales Image", type=['jpg', 'png', 'jpeg'])
...
if uploaded_file:
    ...
    if "621822" in raw_data:
        ...
        for code, ratio in {"231259": 0.5, "654830": 0.3, "724276": 0.2}.items():
            if code not in raw_data: raw_data[code] = {"qty": 0, "amt":
